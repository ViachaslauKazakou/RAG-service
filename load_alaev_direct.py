...
async def load_alaev_data():
    ...
    try:
        ...
        result = await db.execute(
            text("SELECT id FROM user_knowledge WHERE user_id = :user_id"), {"user_id": data["user_id"]}
        )
        ...
        await db.execute(
            text("DELETE FROM user_message_examples WHERE user_id = :user_id"), {"user_id": data["user_id"]}
        )
        ...
        # Проверяем знания
        result = await db.execute(
            text("SELECT name, character_id FROM user_knowledge WHERE user_id = :user_id"), {"user_id": data["user_id"]}
        )
        ...
        # Проверяем сообщения
        result = await db.execute(
            text("SELECT COUNT(*) FROM user_message_examples WHERE user_id = :user_id"), {"user_id": data["user_id"]}
        )
