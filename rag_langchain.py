...
class AdvancedRAG:
    ...
    def _load_cache(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Кеш загружен: {metadata.get('documents_count', 0)} документов из {metadata.get('created_at', 'неизвестно')}"
            )
            return True
    ...
    def _create_new_rag(self):
        ...
        try:
            ...
            for i, doc in enumerate(documents[:5]):  # Первые 5 для примера
                logger.info(
                    f"   📄 Документ {i+1}: source={doc.metadata.get('source', 'unknown')}, length={len(doc.page_content)}"
                )
    ...
    def get_cache_info(self) -> Dict:
        ...
        cache_files = [("vectorstore", self.vectorstore_cache_file), ("metadata", self.metadata_cache_file)]
    ...
    def add_documents(self, new_documents: List[str], sources: Optional[List[str]] = None):
        ...
        try:
            ...
            for i, doc_text in enumerate(new_documents):
                ...
                doc = Document(
                    page_content=doc_text, metadata={"source": source, "added_at": datetime.now().isoformat()}
                )
        ...
        try:
            ...
            for doc in source_docs:
                if (
                    ...
                    and doc.page_content != main_content
                    and len(context_chunks) < window
                ):
    ...
    def create_extended_documents_from_json(self, use_extended_parsing: bool = True) -> None:
        ...
        try:
            ...
            for file_path in json_files:
                ...
                try:
                    ...
                    for i, message in enumerate(messages):
                        ...
                        logger.info(
                            f"      📄 Создан документ {i+1}: {message.get('character')} - {message.get('content', '')[:50]}..."
                        )
            ...
            self.retriever = self.vectorstore.as_retriever(
                search_type="similarity", search_kwargs={"k": 5}  # Увеличиваем k для большего числа документов
            )
